...
flirt -applyxfm -init ${FSLDIR}/etc/flirtsch/ident.mat -in ${ANALYSISDIR}_lesion_masks/${MYSUB}-${EXAM}/anat/T1_lesion_mask_filled -out ${ANALYSISDIR}_lesion_masks/${MYSUB}-${EXAM}/anat/T1_lesion_mask_filled2mT1 -ref ${ANALYSISDIR}-${EXAM}/${MYSUB}/anat/mT1 -interp nearestneighbour
...
EXAM=$pre_exam
mkdir ${ANALYSISDIR}/${MYSUB}_${EXAM}/anat/xfms
mkdir ${ANALYSISDIR}/${MYSUB}_${EXAM}/anat/xfms/ants
mkdir ${ANALYSISDIR}/${MYSUB}_${EXAM}/anat/xfms/ants/bet
...
antsRegistration --dimensionality 3 --float 0 --output [MNI_1mm_2_mT1,MNI_1mm_2_mT1_Warped.nii.gz] --interpolation Linear  --winsorize-image-intensities [0.005,0.995]  --use-histogram-matching 0 --initial-moving-transform [${ANALYSISDIR}/${MYSUB}-${EXAM}/anat/xfms/ants/mT1_brain.nii.gz,${ANALYSISDIR}/MNI152_T1_1mm_target.nii.gz,1] --transform Rigid[0.1] --metric MI[${ANALYSISDIR}/${MYSUB}-${EXAM}/anat/xfms/ants/mT1_brain.nii.gz,${ANALYSISDIR}/MNI152_T1_1mm_target.nii.gz,1,32,Regular,0.25] --convergence [1000x500x250x100,1e-6,10] --shrink-factors 8x4x2x1 --smoothing-sigmas 3x2x1x0vox --transform Affine[0.1] --metric MI[${ANALYSISDIR}/${MYSUB}-${EXAM}/anat/xfms/ants/mT1_brain.nii.gz,${ANALYSISDIR}/MNI152_T1_1mm_target.nii.gz,1,32,Regular,0.25] --convergence [1000x500x250x100,1e-6,10]  --shrink-factors 8x4x2x1 --smoothing-sigmas 3x2x1x0vox --transform SyN[0.1,3,0] --metric CC[${ANALYSISDIR}/${MYSUB}-${EXAM}/anat/xfms/ants/mT1_brain.nii.gz,${ANALYSISDIR}/MNI152_T1_1mm_target.nii.gz,1,4] --convergence [100x70x50x20,1e-6,10] --shrink-factors 8x4x2x1  --smoothing-sigmas 3x2x1x0vox ${INMASK}
