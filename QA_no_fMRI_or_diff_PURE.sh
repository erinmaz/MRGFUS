...
fsl_motion_outliers -i ${ANALYSISDIR}/${MYSUB}/fmri/PUrs -s ${ANALYSISDIR}/${MYSUB}/fmri/PUmissing_rs_FD.rms -p ${ANALYSISDIR}/${MYSUB}/fmri/PUrs_FD.png --thresh=2 -o ${ANALYSISDIR}/${MYSUB}/fmri/PUmissing_rs_FD_confounds.txt
...
################# SUMMARY OUTPUT ########################################
echo ${MYSUB}_no_fMRI_or_diff_PURE $PUdwtsnr $PUrstsnr $rstsnr `awk -v max=0 '{if($1>max){ max=$1}}END{print max} ' ${ANALYSISDIR}/${MYSUB}/fmri/PUrs_FD.rms` `awk '{ total += $1 } END { print total/NR}' ${ANALYSISDIR}/${MYSUB}/fmri/PUrs_FD.rms`
