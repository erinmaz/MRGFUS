...
#FLIPS RIGHT HEMI LESIONS INTO LEFT HEMISPHERE
...
RUNS=( 9001_SH-11644 9001_SH-11692 9001_SH-12271 9001_SH-14297 9002_RA-11764 9002_RA-11833 9002_RA-12388 9002_RA-14439 9004_EP-12203 9004_EP-12126 9004_EP-12955 9005_BG-13004 9005_BG-13126 9005_BG-13837 9006_EO-12389 9006_EO-12487 9006_EO-13017 9007_RB-12461 9007_RB-12910 9007_RB-13055 9009_CRB-12609 9009_CRB-13043 9009_CRB-13623 9013_JD-13455 9013_JD-13722 9013_JD-14227 )

SUBS=( 9001_SH 9002_RA 9004_EP 9005_BG 9006_EO 9007_RB 9009_CRB 9013_JD )

SIDE=( L L L R L R L L )

#currently only 4 or 3 will work, but easy enough to add more (see below)
NUM_SESS=( 4 4 3 3 3 3 3 3 )
...
for f in "${RUNS[@]}"
do
    echo $sess $index ${NUM_SESS[${index}]}
	cp ${ANALYSISDIR}/${f}/diffusion/dtifit_FA.nii.gz ${MAINDIR}/${TBSSNAME}/${f}_FA.nii.gz
	if [ "${SIDE[${index}]}" == "R" ]; then
	  echo $f
	  fslswapdim ${MAINDIR}/${TBSSNAME}/${f}_FA.nii.gz -x y z ${MAINDIR}/${TBSSNAME}/${f}_FA.nii.gz
	fi
	if [ "${NUM_SESS[${index}]}" == "${sess}" ]; then
	  let index=$index+1
	  let sess=1
	else
	  let sess=$sess+1
